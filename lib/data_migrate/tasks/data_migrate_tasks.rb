  ...
  module Tasks
    module DataMigrateTasks
      ...
      def smart_migrate
        ...
        target_version = ENV["VERSION"] ? ENV["VERSION"].to_i : nil
        if Rails::VERSION::MAJOR == 5 && Rails::VERSION::MINOR == 2
          DataMigrate::MigrationContext.new(migrations_paths).smart_migrate(target_version)
        else
          paths = migrations_paths || "db/data/"
          DataMigrate::DataMigrator.smart_migrate(paths, ENV["VERSION"] ? ENV["VERSION"].to_i : nil)
        end
      end
